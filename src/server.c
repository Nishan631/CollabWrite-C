...
/* Helper function to save trie words */
void save_trie_recursive(FILE *f, TrieNode *node) {
    if (!node) return;
    if (node->is_word && node->word) {
        fprintf(f, "%s\n", node->word);
    }
    for (int i = 0; i < ALPHABET_SIZE; ++i) {
        if (node->children[i]) save_trie_recursive(f, node->children[i]);
    }
}

void save_trie_to_file(const char *filename) {
    FILE *f = fopen(filename, "w");
    if (!f) {
        perror("Error saving dictionary");
        return;
    }
    extern TrieNode *get_trie_root(); // if root is static, you can expose it via a getter in trie_suggest.c
    save_trie_recursive(f, get_trie_root());
    fclose(f);
    printf("[SERVER] Dictionary saved to %s.\n", filename);
}
...
void handle_command(char *line, int sock) {
    ...
    // --- NEW: append new words to words.txt immediately ---
    FILE *dict = fopen("words.txt", "a");
    if (!dict) {
        perror("Unable to open words.txt for appending");
        // still continue; trie will have the word in memory
    }
    ...
    while (text[i]) {
        if (isalpha((unsigned char)text[i])) {
            ...
            if (j >= (int)sizeof(word) - 1) j = sizeof(word)-2; // guard
        } else {
            if (j > 0) {
                word[j] = '\0';
                if (!trie_contains(word)) {
                    trie_insert(word, 1);            // new: give initial freq 1
                    if (dict) fprintf(dict, "%s\n", word);
                } else {
                    trie_insert(word, 0);            // existing: increment freq
                }
                j = 0;
            }
        }
    ...
    printf("[SERVER] Added words from INS to dictionary and appended new ones to words.txt.\n");
}
    ...
    else if (strncmp(line, "AUTOCOMPLETE ", 13) == 0) {
        char *prefix = line + 13;
        int n;
        Suggestion **s = get_autocomplete(prefix, 5, &n);
        char reply[1024] = "SUGGEST ";
        for (int i = 0; i < n; i++) {
            strcat(reply, s[i]->word);
            if (i != n - 1) strcat(reply, ",");
        }
        strcat(reply, "\n");
        send(sock, reply, strlen(reply), 0);
    ...
    else if (strncmp(line, "AUTOCORRECT ", 12) == 0) {
        char *word = line + 12;
        int n;
        Suggestion **s = get_autocorrect(word, 2, 3, &n);
        char reply[1024] = "CORRECT ";
        for (int i = 0; i < n; i++) {
            strcat(reply, s[i]->word);
            if (i != n - 1) strcat(reply, ",");
        }
...
int main() {
    ...
    if (f) fclose(f);
    printf("[SERVER] Dictionary loaded from words.txt.\n");
